...
$assignTeachers = $conn->query("
    SELECT 
        ...
        GROUP_CONCAT(DISTINCT teacher_subjects.schedule_day ORDER BY teacher_subjects.schedule_day ASC SEPARATOR ', ') AS schedule_days,
        GROUP_CONCAT(DISTINCT CONCAT(teacher_subjects.schedule_time_start, ' - ', teacher_subjects.schedule_time_end) ORDER BY teacher_subjects.schedule_time_start SEPARATOR ', ') AS schedule_times
    FROM teacher_subjects
    JOIN teachers ON teacher_subjects.teacher_id = teachers.t_id
    JOIN subjects ON teacher_subjects.subject_id = subjects.s_id 
    GROUP BY teachers.t_name, subjects.s_course_code, subjects.s_descriptive_title
                            ...
                            <?php foreach ($assignTeachers as $assign): ?>
                                <tr>
                                    ...
                                    <td style="font-size: 14px;">
                                        <?= $assign['schedule_days'] ?> (<?= $assign['schedule_times'] ?>)
                                    </td>
                                    <td>
                                        <div class="action">
                                            <button type="button" class="btn btn-primary update"
                                                data-bs-toggle="modal" data-bs-target="#assignEditTeacher"
                                                data-id="<?= $assign['id'] ?>" data-subject-id="<?= $assign['subject_id'] ?>">
                                                <i class="fa-solid fa-pencil-alt"></i>
                                            </button>
                                            <button type="button" class="btn btn-warning view"
                                                data-bs-toggle="modal" data-bs-target="#assignViewTeacher"
                                                data-id="<?= $assign['id'] ?>">
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll(".delete-assign").forEach(button => {
                button.addEventListener("click", function() {
                    ...
                    Swal.fire({
                        title: "Are you sure?",
                        text: "Deleting this will also remove associated student grades (if any)!",
                        icon: "warning",
                        ...
                        if (result.isConfirmed) {
                            fetch("delete_assign_teacher.php", {
                                    ...
                                    body: `assign_id=${assignId}&subject_id=${subjectId}`
                                })
                                .then(response => response.json())
                                .then(data => {
                                    if (data.success) {
                                        Swal.fire({
                                            title: "Deleted!",
                                            text: data.message || "The teacher assignment and related student grades have been removed.",
                                            icon: "success",
                                            timer: 2000,
                                        ...
                                        Swal.fire("Error!", data.message || "Failed to delete the teacher assignment.", "error");
                                    }
                                })
                                .catch(error => {
